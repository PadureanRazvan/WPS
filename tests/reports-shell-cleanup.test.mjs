...
test('reports shell delegates report calculation to the Report Read Model', () => {
  assert.match(source, /import\s+\{\s*buildReportReadModel\s*\}\s+from\s+'\.\/report-read-model\.js\?v=2026\.07\.15\.15';/);
...
test('reports shell delegates report HTML to the Reports View Module', () => {
  assert.match(source, /import\s+\{\s*renderReportsView\s*\}\s+from\s+'\.\/reports-view\.js\?v=2026\.07\.15\.15';/);
