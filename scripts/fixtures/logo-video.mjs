...
export async function recordLogoVideo(page, path, { selector, seconds, transitionAt = null, autoCycle = false }) {
    const result = await page.evaluate(async ({ selector, seconds, transitionAt, autoCycle }) => {
        ...
        // App body backgrounds can be transparent over a CSS scene. Resolve
        // the theme's opaque surface and clear each frame to avoid trails.
        const background = getComputedStyle(source).getPropertyValue('--surface-elevated').trim() || '#ffffff';
        ...
        controllers.forEach(controller => { controller.setAutoCycle(autoCycle); controller.resume(); });
        ...
        const shapesSeen = [...new Set(costs.map(frame => frame.shape))];
        ...
        return { dataUrl, frames, simulatedSeconds: seconds, wallSeconds: (Date.now() - started) / 1000,
            width: canvas.width, height: canvas.height, background,
            ...
            slowestFrames: costs.slice(-5).reverse(), shapesSeen };
    }, { selector, seconds, transitionAt, autoCycle });
